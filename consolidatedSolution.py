...
for dic in videoTextArray:
    ...
    file_exists2 = os.path.exists(f"{textFileName}.csv")

    # Append the DataFrame to the CSV file without removing existing data
    if not file_exists2:
        df2.to_csv(
            f"{textFileName}.csv", mode="w", header=True, index=False, encoding="utf-8"
        )
    else:
        df2.to_csv(
            f"{textFileName}.csv", mode="a", header=False, index=False, encoding="utf-8"
        )

    print(f"Data written to {textFileName}.csv file...")
