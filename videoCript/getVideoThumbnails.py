    ...
    for index in range(len(updatedThumbnailUrls)):
        for updatedUrl in updatedThumbnailUrls:
            if f"thumbnail{(index+1) * time_interval}.jpg" in updatedUrl:
                thumbnails_with_times.append({"thumbnail_url": updatedUrl, "frame_time": (index+1) * time_interval,"title":title, "video_id":videoId, "video_download_url":downloadUrl})
